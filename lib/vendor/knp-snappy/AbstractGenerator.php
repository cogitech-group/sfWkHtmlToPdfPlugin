...
abstract class AbstractGenerator implements GeneratorInterface
{
    ...
    public function setOption($name, $value)
    {
        if (!array_key_exists($name, $this->options)) {
            throw new \sfException(sprintf('The option \'%s\' does not exist.', $name));
        }
    ...
    {
        if (array_key_exists($name, $this->options)) {
            throw new \sfException(sprintf('The option \'%s\' already exists.', $name));
        }
    ...
    protected function createTemporaryFile($content = null, $extension = null)
    {
        ...
        if (!is_dir($dir)) {
            if (false === @mkdir($dir, 0777, true) && !is_dir($dir)) {
                throw new \sfException(sprintf("Unable to create directory: %s\n", $dir));
            }
        } elseif (!is_writable($dir)) {
            throw new \sfException(sprintf("Unable to write in directory: %s\n", $dir));
        }

        $filename = $dir . DIRECTORY_SEPARATOR . uniqid('wkhtmltopdf_', true);
